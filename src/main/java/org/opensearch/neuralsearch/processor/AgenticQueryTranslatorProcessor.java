...
public class AgenticQueryTranslatorProcessor extends AbstractProcessor implements SearchRequestProcessor {
    ...
    private void executeAgentAsync(
        ...
        // Get index names from the search request
        if (request.indices() != null && request.indices().length > 0) {
            try {
                parameters.put("index_name", Arrays.toString(request.indices()));
            } catch (Exception e) {
                log.warn("Failed to get index names", e);
            }
        }
        ...
        // First get agent type and system prompt info
        mlClient.getAgentType(agentId, ActionListener.wrap(agentInfo -> {
            ...
            mlClient.executeAgent(agentId, agentType, hasSystemPrompt, xContentRegistry, parameters, ActionListener.wrap(agentResponse -> {
                try {
